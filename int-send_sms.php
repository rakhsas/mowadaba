...
*/
//$post_body = seven_bit_sms( $seven_bit_msg);
...
*/
$post_body1 = unicode_sms($unicode_msg);
$post_body = array('id'=>'123','phone'=>'0673697101','message'=>"gggg");
$result = send_message( $post_body, $url );
if( $result['success'] ) {
  print_ln( formatted_server_response( $result ) );
}
...
*/
/*$post_body = eight_bit_sms( $eight_bit_msg);
$result = send_message( $post_body, $url );
if( $result['success'] ) {
  print_ln( formatted_server_response( $result ) );
}
...
function seven_bit_sms ( $message) {
  $post_fields = array (
...
*/
function eight_bit_sms( $message ) {
  $post_fields = array (
  'message'  => $message,
  'dca'      => '8bit'
  );
